...
namespace MyEngine_Core.ECS.MyComponents
{
    public class TilemapComponent
    {
        ...
        public TilemapComponent(Tileset tileset, int columns, int rows)
        {
            _tileset = tileset;
            Rows = rows;
            Columns = columns;
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            for (int i = 0; i < Count; i++)
            {
                ...
                // Check if the tileSetIndex is valid (not -1, which we used to indicate an empty tile)
                if (tileSetIndex >= 0)
                {
        ...
        public static TilemapComponent FromFile(ContentManager content, string filename)
        {
            ...
            using (Stream stream = TitleContainer.OpenStream(filePath))
            {
                using (XmlReader reader = XmlReader.Create(stream))
                {
                    ...
                    // Create the tilemap
                    TilemapComponent tilemap = new TilemapComponent(tileset, columnCount, rows.Length);
                    ...
                    for (int row = 0; row < rows.Length; row++)
                    {
                        ...
                        for (int column = 0; column < columnCount; column++)
                        {
                            ...
                            // Check if the tilesetIndex is 0. If it is, do not draw a tile.
                            // We will set a value like -1 to indicate an empty tile, as 
                            // the tileset indices are 0-based, and we want 1-based indexing
                            // for non-empty tiles.
                            if (tilesetIndex > 0)
                            {
                                // Adjust the index to be 0-based for the tileset
                                tilemap.SetTile(column, row, tilesetIndex - 1);
                            }
                            else
                            {
                                // For a value of 0 in the XML, set an invalid index to signal no tile
                                // should be drawn.
                                tilemap.SetTile(column, row, -1);
                            }
                        }
                    }
